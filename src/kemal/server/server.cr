...
module Kemal
  class Server
    ...
    def start
      ...
      if ssl_enabled?
        ssl_config = Kemal::SSL.new(settings.ssl_key_file.not_nil!, settings.ssl_cert_file.not_nil!).generate_tls
        server.bind_tls Kemal.settings.host, Kemal.settings.port, ssl_config, settings.port_reuse
      else
        server.bind_tcp Kemal.settings.host, Kemal.settings.port, settings.port_reuse
      end
      ...
      loop do
        begin
          Log.info { "Server started in #{Kemal.env.colorize(:yellow)}." }
