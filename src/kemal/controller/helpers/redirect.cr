module Kemal::Controller::Helpers
  ...
  private class Redirector
    ...
    def self.from_controller_action(controller : Class, action : Symbol, **options)
      route = Kemal::Server.router.match_by_controller_action(controller.to_s.downcase, action)
      redirect(route, **options)
    end

    def self.from_controller_action(controller : Symbol | String, action : Symbol, **options)
      route = Kemal::Server.router.match_by_controller_action("#{controller}controller", action)
      redirect(route, **options)
    end
